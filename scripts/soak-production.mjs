...
const tag = process.argv[3] ?? 'v06';
const browser = await chromium.launch({ headless: true });
const context = await browser.newContext({ viewport: { width: 1440, height: 1000 } });
const page = await context.newPage(), cdp = await context.newCDPSession(page);
const errors = [], failed = [], checkpoints = [];
...
try {
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    if (errors.length || failed.length) throw new Error('Browser errors in soak');
  ...
  const report = { passed, rounds, deaths, roundsWithReward, production: true, devHandleAbsent: true, seconds: (Date.now() - started) / 1000,
    growth, checkpoints, errors, failed, note: 'Heap/DOM checkpoints follow forced GC; this is retained-growth evidence, not a peak-memory or low-end-device benchmark.' };
  ...
  await writeFile(`.artifacts/${tag}-production-soak.json`, JSON.stringify({ failure: error.message, checkpoints, errors, failed }, null, 2));

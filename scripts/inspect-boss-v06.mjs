...
try {
  ...
  await page.evaluate(() => {
    ...
    for (let i = 0; i < 16; i++) {
      const x = 70 + i % 4 * 155, y = 79 + Math.floor(i / 4) * 88;
      s.add.image(x, y, 'boss-full', 16 + i).setOrigin(.5, 112 / 128).setDepth(101);
      s.add.graphics().setDepth(102).lineStyle(1, 0x7ab4a7, .4).strokeRect(x - 22, y - 62, 44, 62);
      s.add.text(x + 50, y - 4, String(16 + i), { fontSize: '9px', color: '#c3cbb6' }).setDepth(102);
    }
    document.getElementById('hud').hidden = true;
    document.getElementById('toast').hidden = true;
  });
  await page.screenshot({ path: '.artifacts/v06-boss-poses.png' });

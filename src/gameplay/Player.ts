...
export interface PlayerCombatState { phase: 'idle' | 'windup' | 'active' | 'recovery'; facing: 1 | -1; lunge: number; progress: number; }
...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.setDisplaySize(44, 55);
    // Body sizes are source pixels; the offset keeps the body bottom aligned to the imported feet.
    // The offset keeps its collision bottom aligned to the imported sprite's feet (frame y=77).
    (this.body as Phaser.Physics.Arcade.Body).setSize(30, 60, false).setOffset(17, 18);
  ...
  updateMovement(input: PlayerInput, deltaMs: number, allowDash: boolean, combat: PlayerCombatState): PlayerStep {
    ...
    const combatLean = combat.phase === 'windup' ? -7 + combat.progress * 5
      : combat.phase === 'active' ? -2 + combat.progress * 10
        : combat.phase === 'recovery' ? 8 - combat.progress * 8 : 0;
    let dashed = false;
    let dashEnded = false;
    if (this.dashMs > 0) {
    ...
    this.setAngle(this.facing * combatLean);

    this.updateAnimation(grounded, body.velocity.x);
  ...
  beginAttack(): void { this.attackPhase = 'windup'; this.play('hero-attack', true); }
  ...
  private updateAnimation(grounded: boolean, velocityX: number): void {
    if (this.attackPhase !== 'idle') this.play('hero-attack', true);
    else if (!grounded) this.play('hero-jump', true);

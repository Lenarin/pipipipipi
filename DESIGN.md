...
Version 0.5 P0: attacks use eight authored full-body poses and 330/370/520-ms phase schedules. Immediate input registration is separate from finite one-action buffering. Attack-only forward impulse decays and cannot drive the hero through the target; walking remains unchanged. Light recoil keeps targets available to the next stroke, while the finisher gives a stronger decaying push. Pipe trails sit behind actors; compact impact flashes sit below overhead danger cues. Hitstop is 50/55/85 ms, once per swing, with input still accepted. Avoid impact effects on misses and avoid sound/stop stacking when hitting groups. P1 locomotion, landing and enemy-death redesign are deferred.
